...
const KAIGO_FILE = path.join(__dirname, 'kaigo_raw.json');
...
async function processKaigo() {
    console.log("Loading raw Kaigo data...");
    const rawData = JSON.parse(fs.readFileSync(KAIGO_FILE, 'utf8'));

    // Check duplicates against existing wam cards to avoid redundancy
    const existingTerms = new Set(existingCards.map(c => c.term));
    const newCards = [];

    // Filter out duplicates and invalid entries first
    const candidates = rawData.filter(item => {
        if (!item.term || !item.original_definition) return false;
        if (item.original_definition.trim().startsWith('→')) return false; // Skip redirects
        if (existingTerms.has(item.term)) return false; // Skip duplicates
        return true;
    });

    console.log(`Processing ${candidates.length} new terms from Kaigo Jiten...`);
    ...
    for (const item of candidates) {
        ...
        if (newDef) {
            newCards.push({
                id: `kaigo_${processedCount + 1}`,
                term: item.term,
                definition: newDef,
                group_id: 'kaigo_glossary',
                category_label: '介護用語辞典',
