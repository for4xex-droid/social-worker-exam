...
def clean_text(text):
    ...
    # Remove standalone numbers that look like page numbers (appearing at start/end or floating)
    # Be careful not to remove numbers in "平成24年" etc.
    # Pattern: Space/Newlines + digits + Space/Newlines
...
def process_file(filepath):
    ...
    for item in data:
        ...
        # Also clean options if they are strings
        if "options" in item:
            opts = item["options"]
            if isinstance(opts, list):
                new_opts = []
                for opt in opts:
                    new_opts.append(clean_text(opt))
