...
You are the Lead Architect for the "Welfare Master" project. You must strictly adhere to these rules to ensure the success of the "One Codebase, Multi Apps" strategy.
...
*   **No Magic Strings**: Use constants or enums for `group_id` filtering (e.g., `GROUP.PAST_SOCIAL`).
...
*   **Expo SDK & Native Modules**: Always match JavaScript versions to the native versions bundled with the user's Expo Go. 
    *   *Worklets Mismatch*: If `Mismatch between JS part and native part of Worklets` occurs, verify and force-match the version in `package.json` to exactly what the mobile Go app provides (e.g., 0.5.1).
*   **Babel & Metro**: `metro.config.js` MUST be correctly configured for NativeWind to scan files. An empty config will lead to silent styling failures.
*   **Development Toggles**: Use a global `SHOW_ONBOARDING_FLOW` constant in `_layout.tsx` to bypass repetitive UX flows during rapid development.
...
---
**Verified & Locked by User request.**
